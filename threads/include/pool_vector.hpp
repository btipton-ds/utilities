...
TEMPL_DECL
VECTOR_DECL::vector(local_heap& heap)
: _pHeap(&heap)
{
}

TEMPL_DECL
VECTOR_DECL::vector(const std::vector<T>& src, local_heap& heap)
: _pHeap(&heap)
{
...
TEMPL_DECL
VECTOR_DECL::vector(const std::initializer_list<T>& src, local_heap& heap)
: _pHeap(&heap)
{
insert(end(), src.begin(), src.end());
}
